...
allowed = function(url, parenturl)
  if string.match(url, "'+")
      or string.match(url, "[<>\\%*%$;%^%[%],%(%){}]")
      or string.match(url, "^https?://trackback%.blogsys%.jp/") then
    return false
  end
  ...
  if string.match(url, "^https?://[^/]*blogimg%.jp") 
    or string.match(url, "^https?://[^/]*blogsys%.jp")
    or string.match(url, "^https?://img%.dlsite%.jp")
    or string.match(url, "^https?://media%.dlsite%.com")
    or string.match(url, "^https?://parts%.blog%.livedoor%.jp") then
    return true
  end

  if not string.match(url, "^https?://blog%.dlsite%.com")
    and not string.match(url, "^https?://b%.[^/]*dlsite%.net") then
    return false
  end

  for s in string.gmatch(url, "(RG[0-9]+)") do
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  local match1, match2 = string.match(url, "(https?://[^/]*blogimg%.jp/[^/]+/imgs.*/[a-z0-9A-Z]+).*(%.[a-z0-9A-Z]+)$")
  if match1 ~= nil then
    check(match1 .. match2)
    check(match1 .. "-s" .. match2)
  end
